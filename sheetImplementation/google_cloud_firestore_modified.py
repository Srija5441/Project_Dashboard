...
def get_all_fragment_urls():
	...
	first_query = col_ref.order_by("url").limit(1000)
	...
	while True:
		...
		for frag in docs:
			...
			last_pop = data
		if docs_left:
			next_query = col_ref.order_by("url").start_after({u'url': last_pop}).limit(1000)
			docs = next_query.get()
		if not docs_left:
			break
	return urls
...
def get_jobs_dict_by_user():
	...
	first_query = jobs_ref.order_by(u'user').limit(1000)
	docs = first_query.get()
	jobs = {}
	...
	while True:
		...
		next_query = jobs_ref.order_by(u'user').start_after({
				u'fragment': last_pop,
	...
	# set GOOGLE_APPLICATION_CREDENTIALS=E:\Project\sheetImplementation\audiotranslation-f775a-firebase-adminsdk-7ouyp-b4d755f6b4.json
